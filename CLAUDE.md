...
- **内容**: 遠隔画像診断・DX支援
- **対象**: クリニック・診療所
- **技術**: 仮想化（VMware）＋RDP＋ネットワーク接続制御の具体方式によるセキュアな遠隔読影システム
...
- **現行実装**: VMware仮想化＋RDP＋ネットワーク接続制御の具体方式（USB-Ethernetアダプタ）
- **ネットワーク接続制御**: 具体的な仮想化設定・接続制御パラメータは非公開情報として扱う
- **ネットワーク分離の具体方式維持**: Windowsグループポリシーでデバイスインストール禁止＋スタートアップbatによる自動復帰
...
- 技術補足説明書: 作成済み（ネットワーク分離の具体方式の誤解解消・KVM設計・実証結果に基づく運用方針等を補足）
...
| 技術補足説明書_v2.docx | 先行技術調査報告へのコメント・ネットワーク分離の具体方式の技術詳細 |
